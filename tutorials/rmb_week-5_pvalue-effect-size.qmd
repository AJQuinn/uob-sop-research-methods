...
Please note, this is the same dataset as we used in week 4 - but make sure that you load in the `rmb-week-3_lecture-quiz-data_ai-faces-2026.omv` file from this week and NOT the week 4 file. It contains a small fix that we need for this week.
...
Levene's test for homogeneity of variance `r mcq(c(answer = "no significant", 'a significant'))` difference in the variances of the two groups, F(`r fitb(1)`,`r fitb(58)`) = `r fitb(0.018)`, p = `r fitb(0.893)`.
...
The effect size `-0.81` is just below the thresold to be a 'medium' effect (ignoring the sign of the effect size). The effect size for the Rank Biserial correlation is similar.
...
The effect size `-0.02` is a very 'small' effect (we can ignore the sign of the effect size). The effect size for the Rank Biserial correlation is much larger - perhaps as the data is not normally distributed.
...
The effect size `-0.083` is tiny! the Rank Biserial correlation is even smaller...
