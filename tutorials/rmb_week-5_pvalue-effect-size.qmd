...
The effect size `-0.429` is just below the thresold to be a 'medium' effect (ignoring the sign of the effect size). The effect size for the Rank Biserial correlation is similar.
...
How many participants would we need to have 80% chance of correctly detecting the estimated effect size from Cohen's d for `ChangeEmoConfidence`?
