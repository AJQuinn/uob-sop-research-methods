...
We'll think about two data transformations in this section

| Transform | Explanation |
| --------- | ---------------------------- |
| Log Transform | The logarithm function, typically base 10 (log), is applied to each data point. |
| Box-Cox Transform | Similar to the log-transform but with some parameters are tuned to work optimally for the specific dataset |
...
This will open a dialogue box where you cna define the new variable. Give your new variable a descriptive name and make sure that the 'Source variable' is the one that you expect. Note that you might see a 'Circular definition' warning until you rename your variable.
...
![](../images/rmb-week-7_computer-practical/rmb-week-7_computer-practical_img15.png){width=450px}
...
![](../images/rmb-week-7_computer-practical/rmb-week-7_computer-practical_img16.png)
...
With the original data, a Shapiro-Wilk statistic found a `r mcq(c(answer="a significant", "no significant"))` difference from a normal distribution, W=`r fitb(0.881)`, p = `r fitb(0.001)`. We can proceed with `r mcq(c("Student's t-test", answer="Mann-Whitney U test"))`.

Using the log-transformed data, a Shapiro-Wilk statistic found a `r mcq(c("a significant", answer="no significant"))` difference from a normal distribution, W=`r fitb(0.974)`, p = `r fitb(0.550)`. Levene's test found that the we `r mcq(c(answer="can", "can not"))` assume that the groups have homogeneity of variance, F(1, 34) = `r fitb(0.357)`, p = `r fitb(0.554)`. We can proceed with `r mcq(c(answer="Student's t-test", answer="Welch's t-test", "Mann-Whitney U test"))`.

Using the BoxCox-transformed data, a Shapiro-Wilk statistic found a `r mcq(c("a significant", answer="no significant"))` difference from a normal distribution, W=`r fitb(0.982)`, p = `r fitb(0.826)`. Levene's test found that the we `r mcq(c(answer="can", "can not"))` assume that the groups have homogeneity of variance, F(1, 34) = `r fitb(0.634)`, p = `r fitb(0.431)`. We can proceed with `r mcq(c(answer="Student's t-test", answer="Welch's t-test", "Mann-Whitney U test"))`.

## Hint

![](../images/rmb-week-7_computer-practical/rmb-week-7_computer-practical_img17.png)
