...
The columns containing the responses for each face have a `0` when the participant was correct and a `1` when the participant was correct for that face.
...
Confidence in indentifying people you used to know in primary school from strangers  `r mcq(c('decreased', answer = 'increased'))` from `r fitb(3.51)` before to `r fitb(3.63)` afterwards.
...
The least accurate participant got a proportion of `r fitb(0.25)` correct, this corresponds to `r fitb(4)` correct responses out of a possible 12.
